...
def test_solve_cli_accepts_nearest_2opt_relocate_candidate_limited_method(tmp_path):
    input_path = tmp_path / "check.pkl"
    output_path = tmp_path / "predictions_candidate_limited.json"
    raw_instances = [
        ([[0, 0]], [[1, 0], [10, 0], [11, 0]], [1, 1, 1], 3),
    ]
    with input_path.open("wb") as handle:
...
def test_solve_cli_help_describes_candidate_limited_relocate_method():
    result = subprocess.run(
        [
            sys.executable,
            str(ROOT / "solve.py"),
...
def test_solve_cli_accepts_nearest_2opt_relocate_limited_swap_method(tmp_path):
    input_path = tmp_path / "check.pkl"
    output_path = tmp_path / "predictions_swap.json"
    raw_instances = [
        ([[0, 0]], [[0, 1], [0, 2], [10, 1], [10, 2]], [1, 1, 1, 1], 2),
    ]
    with input_path.open("wb") as handle:
        pickle.dump(raw_instances, handle)
...
def test_solve_cli_help_describes_limited_swap_method():
    result = subprocess.run(
        [
            sys.executable,
            str(ROOT / "solve.py"),
